...
// drafts); ntok in 1..6 selects how many are live (brace inits with fewer
// entries leave the rest null, unread). P12b: 6 slots so gated depth can reach
// maxd=5 (pending + 5 drafts); depth-4 fills only the first 5.
struct P3 { float* p[6]; };
struct CP3 { const float* p[6]; };
struct XQ3 { XQuant q[6]; };
